...
2. Sends them along with the user's stored selfie/full-body photo to a **Node/TypeScript** backend.
...
| Backend | **Node 20 + TypeScript + Fastify** | One language across extension and backend, fast local iteration |
...
| Dev | Node 20, pnpm, eslint, TypeScript, Vitest | — |
...
│   └── src/
│       ├── app.ts           ← Fastify app + CORS for chrome-extension://
│       ├── server.ts        ← local server entry
│       ├── services/        ← orchestration (understand → generate)
│       ├── providers/       ← base.ts, gemini.ts (later: huggingface.ts)
...
- [ ] Install Node 20, pnpm, VS Code extensions (ESLint, Prettier).
...
1. **Init Node project**
   ...
   - Create `package.json`, `tsconfig.json`, `.env.example`, and `src/`.
   - Use Fastify for HTTP, Zod for validation, Sharp for image processing, and Vitest for tests.
2. **Zod schemas** (`src/schemas/tryon.ts`)
...
3. **Provider interface** (`src/providers/base.ts`)
   - `interface TryOnProvider { generate(input): Promise<TryOnGeneration> }`
4. **Gemini provider** (`src/providers/gemini.ts`)
   - Use `@google/genai`.
   - Call `client.models.generateContent(...)` with prompt, user image, and product image parts.
...
5. **Orchestration service** (`src/services/tryon-service.ts`)
   - Validate inputs, downscale images to max 1024px with Sharp, call provider, time the call.
6. **Routes** (`src/app.ts`)
   - `POST /api/tryon` → accepts JSON with base64 user image + product image URL.
   - Fetch product image server-side with `fetch` (avoids CORS on extension side).
7. **CORS** for `chrome-extension://*` in `src/app.ts`.
8. **Local test**: `pnpm dev`, hit `/api/tryon` with a saved selfie + Amazon image URL via `curl` or Postman.
9. **Unit tests** (`src/tests/app.test.ts`) — mock image fetching, assert schema validation, error paths.
...
- [ ] GitHub Actions CI: lint + backend tests + extension build.
...
| CORS / Amazon hotlink protection on images | Low | Fetch product image **server-side** in the Node backend, not from content script |
...
3. Start **Phase 1, step 1** (init Node backend + install deps).
